...
import { resenTokenSchema, TResenToken } from "@/types/authType"
...
const ResentTokenPage = () => {
  const { register, handleSubmit, formState: { errors } } = useForm<TResenToken>({
    resolver: zodResolver(resenTokenSchema)
  });
  ...
  const onSubmit = handleSubmit((formData: TResenToken) => mutate(formData))
